...
(function() {
	...
	function ajax(method, url, data, callback, errorHandler) {
		var xhr = new XMLHttpRequest(); // browser 发送 request的object
		// 需要对这个object进行一些设置，比如把request发送到哪里
		// request 往哪里走
		// true: 发送的是asynchronize request
		xhr.open(method, url, true); 
		// configure:
		// request回来之后进行处理
		xhr.onload = function() { 
			if (xhr.status === 200) {
				callback(xhr.responseText); // 返回 JSON response
	...
	function loadFavoriteItems() {
		...
		ajax("GET", url + "?" + params, req, function(res) {
			...
			showErrorMessage("Cannot load Favorite item.");
		});
	}
})();
